...
A signed distance functions (SDF) passes spatial coordinates and returns a signed value representing the shortest distance to the boundary of a spatial domain $\Omega$. Mathematically, the signed distance function $$f: \mathbb{R^n} \mapsto \mathbb{R}$$ of a subset $$\Omega \subset \mathbb{R}^n$$ is defined by
$$
    f(x) := 
\begin{cases}
    d(x,\partial \Omega )   & \text{if } x \in \Omega\\
    -d(x,\partial \Omega ) & \text{if } x \in \Omega/\mathbb{R}
\end{cases}
$$
