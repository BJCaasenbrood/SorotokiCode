classdef NeoHookeanMaterialStable
...
methods  
%--------------------------------------------------------------- Mesh Class
function obj = NeoHookeanMaterialStable(varargin) 
    ...
    obj.Mu     = E0/(2*(1+Nu0));
    obj.Alpha  = 1 + obj.Mu/obj.Lambda - obj.Mu*obj.Lambda/4;
    obj.C10    = obj.Mu/2;
    obj.D1     = obj.Lambda/2;
...
%---------------------------------------------------------------------- get     
function varargout = get(NeoHookeanMaterialStable,varargin)
    if nargin > 1
        varargout{nargin-1,1} = [];
        for ii = 1:length(varargin)
            varargout{ii,1} = NeoHookeanMaterialStable.(varargin{ii});
        end
    else
        varargout = NeoHookeanMaterialStable.(varargin);
    end
end      
%---------------------------------------------------------------------- set
function NeoHookeanMaterialStable = set(NeoHookeanMaterialStable,varargin)
    for ii = 1:2:length(varargin)
        NeoHookeanMaterialStable.(varargin{ii}) = varargin{ii+1};
    end
end
%------------------------------ 2ND PIOLLA STRESSAND STIFFNESS FOR YEOH
function [S, D, P] = PiollaStress(NeoHookeanMaterialStable,F,Robustness)
...
C100 = NeoHookeanMaterialStable.C10; 
K    = NeoHookeanMaterialStable.D1;
A    = NeoHookeanMaterialStable.Alpha;
...
P = C100*(J1 - 3) + K*(J3 - A)^2 - C100*log(J1 + 1);

dJdF = [crossmat(F(:,2))*F(:,3),...
        crossmat(F(:,3))*F(:,1),...
        crossmat(F(:,1))*F(:,2)];

Sp = 2*C100*(1 - 1/(I1 + 1))*F + 2*K*(I3 - A)*dJdF;
