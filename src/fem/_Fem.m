...
h{1} = patch('Faces',FaceMatrix,'Vertices',V,...
    'FaceVertexCData',Z,'Facecolor',S,'LineStyle',Fem.Linestyle,...
    'Linewidth',1.5,'FaceAlpha',1,'EdgeColor','k');

%if Fem.Mesh.get('ShowBoundary')
    h{2} = patch('Faces',BoundMatrix,'Vertices',Fem.Node0,...
...
fprintf('* Solver time horizon = %i (s)\n', Fem.TimeEnd);
fprintf('* Solver time precision = %i (Hz)\n', round(1/Fem.TimeStep));
...
if Fem.Nonlinear
    fprintf('* Nonlinear geometric = true\n');
    fprintf('* Solver time horizon = %i (s)\n', Fem.TimeEnd);
    fprintf('* Solver time precision = %i (Hz)\n', round(1/Fem.TimeStep));
else
    fprintf('* Nonlinear geometric = false\n'); 
end
showMaterialInfo(Fem)
fprintf('--------------------------------------------------------------\n');
end
