function N = VertexNormal(Node,Element)
Fa = Element(:,1); Fb = Element(:,2); Fc = Element(:,3);
...
Angle =  [acos(dot(e1_norm',-e3_norm'));
...
V_norm=sqrt(VerticeNormals(:,1).^2+VerticeNormals(:,2).^2+...
    VerticeNormals(:,3).^2)+eps;
N=VerticeNormals./repmat(V_norm,1,3);
end
