...
%--------------------------------------------------------------- find nodes
function [List] = FindNodes(Mesh,Request)
    List = FindNode(Mesh.Node,Request);
end
%--------------------------------------------------------------- find nodes
function [List] = FindElements(Mesh,Request)
    List = FindNode(Mesh.Center,Request);
end
%-------------------------------------------------------------- END METHODS
end
...
%     else
%         faceEl  = cellfun(@(X) delaunayn(X),nodeset,'UniformOutput',false);
%     end
    if strcmp(Mesh.Type,'C3T8')
        faceDis = cellfun(@(V,F,E) CollectCoplanar(V,F,E),...
         nodeset,faceEl,Mesh.Element,'UniformOutput',false);
    else
        faceDis = cellfun(@(F,E) F(E),...
         Mesh.Element,faceEl,'UniformOutput',false);
    end
