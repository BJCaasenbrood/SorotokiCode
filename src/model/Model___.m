...
for ii = 1:Model.Shapes.NNode
    ...
    % first EL-diff eval
    [K1Z1,K1Z2] = LagrangianODEX(Model,s, x, dx, Z1,NLStiff);
    
    % second EL-diff eval
    [K2Z1,K2Z2] = LagrangianODEX(Model,s+(2/3)*ds, x, dx,...
        Z1 + (2/3)*ds*K1Z1,NLStiff);
...
function [dZ1,dZ2] = LagrangianODEX(Model,s,x,dx,Z1,NLStiff)
...
adXi  = adjointse3(Gamma,U);
adV  = adjointse3(Vw,Vs);

dJ  = A*Model.Shapes.Ba*Theta;
dJt = A*adV*Model.Shapes.Ba*Theta;

% Mtt = zeros(6,6);
% Mtt(1:3,1:3) = Model.Rho*diag(diag(Model.Inertia.Jtt));
% Mtt(4:6,4:6) = Model.Inertia.A*Model.Rho*eye(3);
...
%Dtt = Model.Shapes.Dtt;
deta = -adXi*eta_ + Model.Shapes.Ba*Theta*dx;
...
dK = (Model.Shapes.Ba*Theta).'*Ktt*(Model.Shapes.Ba*Theta);
